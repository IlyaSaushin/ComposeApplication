...
fun NavController.navigate(route: String, params: Bundle?, builder: NavOptionsBuilder.() -> Unit = {}) {
    ...
    Log.d("tag", "navigate: navigated to $route by builder -> $builder")
    navigate(route, builder)
}

fun NavController.navigate(route: String, param: Parcelable, paramKey: String, builder: NavOptionsBuilder.() -> Unit = {}) {
    this.currentBackStackEntry?.arguments?.putParcelable(paramKey, param)
    navigate(route, builder)
}